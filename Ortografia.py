...
def segundaPagina(b, listaCorrecta, listaIncorrecta):
    ...
    puntaje = tk.Label(text="{0}/150".format(intPuntaje),
    ...
    def creacionDeBotones():
        ...
        def comandoOpcion(boton):
            ...
            if(intPosicion == 0 and boton == 1) or (intPosicion == 1 and boton == 2):
                ...
                listaIncorrecta.append(listaCorrecta[intPalabra*2])
                listaIncorrecta.append(listaCorrecta[intPalabra*2+1])
            ...
            puntaje.grid_forget()
            puntaje.configure(text="{0}/500".format(intPuntaje))
            puntaje.grid(column=14, row=2, sticky="E", columnspan=4, padx=10)
            siguiente.configure(state=NORMAL)

            with open(r"C:\Users\Samantha\Desktop\Primer semestre\Pensamiento computacional\Problema reto\PISA\OrtografiaTexto.txt", 'r',  encoding='utf-8-sig') as f:
            ...
            with open(r"C:\Users\Samantha\Desktop\Primer semestre\Pensamiento computacional\Problema reto\PISA\OrtografiaTexto.txt", 'w',  encoding='utf-8-sig') as f:
...
def primeraPagina():
    ...
    for i in range(8):
        listaOpciones.append(tk.Button(text=("Opción", i+1), font=(None, 15), bg="#B13B5B", fg="white", command=lambda: opciones(i)))
        listaOpciones[i].grid(column=(1 + (i % 2)*2), row=(4 + (i//2)*2))

        puntajesLabels.append(tk.Label(text="{0}/500".format(puntajes[i]), font=(None, 15), bg="#E4ADBC"))
